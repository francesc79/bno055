...
static const struct regmap_config bno055_regmap_config = {
	...
	.max_register = BNO055_REG_MAG_RADIUS_MSB,
...
static int bno055_read(struct bno055_data *data, int sensor, int axis, int *val)
{
	int ret;
	__le16 raw_val;
	int reg;
			   ...
			   int *val, int *val2, long mask)
{
	...
	case IIO_CHAN_INFO_RAW:
		...
		case IIO_ANGL_VEL:
			...
			ret = bno055_read(data, GYR, chan->scan_index, val);
			break;
		case IIO_ACCEL:
			/* Accelerometer Unit m/s2 */
			/* Raw value needs to be divied by the 100 to get the exact value.*/
			ret = bno055_read(data, ACC, chan->scan_index, val);
			break;
		case IIO_MAGN:
			/* Magnetometer Unit microTesla */
			/* Raw value needs to be divied by the 16 to get the exact value. */
			ret = bno055_read(data, MAG, chan->scan_index, val);
			break;
		default:
			ret = -EINVAL;
			break;
